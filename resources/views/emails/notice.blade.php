...
<head>
    <meta charset="UTF-8">
    <title>Notice | Santa Fe Water Billing System</title>
</head>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;">
                    ...
                    <tr>
                        <td style="padding:24px;">
                            <p style="margin:0 0 12px 0;">Hello {{ $notice->consumer->first_name }} {{ $notice->consumer->last_name }},</p>
                            <p style="margin:0 0 12px 0;">{{ $prefix }}</p>
                            <h2 style="margin:0 0 12px 0;font-size:18px;">{{ $notice->notice }}</h2>
                            @if(!empty($notice->description))
                                <p style="margin:0 0 16px 0;white-space:pre-line;">{{ $notice->description }}</p>
                            @endif
