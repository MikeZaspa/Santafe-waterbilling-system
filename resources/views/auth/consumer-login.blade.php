...
<script src="https://cdn.datatables.net/responsive/2.4.0/js/responsive.bootstrap5.min.js"></script>
    ...
    $(window).resize(function() {
        ...
        // Add to consumer dashboard JavaScript
class NotificationManager {
    ...
    loadUnreadCount() {
        $.ajax({
            url: '/consumer/notifications/count',
            type: 'GET',
            success: (response) => {
    ...
    updateBadge(count) {
        ...
        const badge = $('#notificationBell .notification-badge');
        
        if (count > 0) {
            if (badge.length === 0) {
                $('#notificationBell').append(`<span class="notification-badge">${count}</span>`);
            } else {
                badge.text(count);
            }
        } else {
    ...
    renderNotifications(notifications) {
        ...
        notifications.forEach(notification => {
            const iconClass = this.getNotificationIcon(notification.data.type);
                ...
                <div class="notification-item ${notification.read_at ? '' : 'unread'}" 
                    ...
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                            <div class="notification-title">${this.getNotificationTitle(notification.data.type)}</div>
    ...
    getNotificationIcon(type) {
        const icons = {
            'billing_created': { icon: 'bi-receipt', class: 'info' },
            'payment_due_reminder': { icon: 'bi-clock', class: 'warning' },
            'payment_confirmed': { icon: 'bi-check-circle', class: 'success' },
            'payment_verified': { icon: 'bi-shield-check', class: 'success' }
        };
        return icons[type] || { icon: 'bi-bell', class: 'info' };
    }

    getNotificationTitle(type) {
        const titles = {
            'billing_created': 'New Bill Generated',
    ...
    getTimeAgo(timestamp) {
        // Implement time ago calculation
    ...
    markAsRead(notificationId) {
        $.ajax({
            url: '/consumer/notifications/mark-read',
            type: 'POST',
            data: { notification_id: notificationId, _token: $('meta[name="csrf-token"]').attr('content') },
            success: () => {
                this.loadUnreadCount();
            }
        });
    ...
    markAllAsRead() {
        $.ajax({
            url: '/consumer/notifications/mark-read',
            type: 'POST',
            data: { all: true, _token: $('meta[name="csrf-token"]').attr('content') },
            success: () => {
